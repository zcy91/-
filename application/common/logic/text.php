...
//         $third_rate = tpCache('distribut.third_rate'); // 三级比例
        ...
        if($pattern  == 0)
        {
            ...
            foreach($order_goods as $k => $v)
            {
                ...
                $first_tmp_commission = $price * ($first_rate /100);
                $second_tmp_commission = $price * ($second_rate /100);
                $third_tmp_commission = $price * ($third_rate /100);
...
//                     }
        ...
        // 如果这笔订单没有分销金额
        if($first_commission == 0 && $second_commission == 0 && $third_commission == 0)
            return false;

//            $first_money = $first_commission * ($first_rate / 100); // 一级赚到的钱
//            $second_money = $second_commission * ($second_rate / 100); // 二级赚到的钱
