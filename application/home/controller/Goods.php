...
class Goods extends Base {
    ...
    */
    public function goodsInfo(){
        ...
        $address = get_user_default_address($user_id);
        $_COOKIE['province_id'] = (int)$address['province'];
        $_COOKIE['city_id'] = (int)$address['city'];
        $_COOKIE['district_id'] = (int)$address['district'];
        ...
        if ($goodsPromFactory->checkPromType($goods['prom_type'])) {
            ...
            //     $goods['flash_sale'] = $goodsPromLogic->checkFlashSaleIsEnd();
            // }
    ...
    public function goodsList(){
        ...
        $filter_goods_id = M('goods')->where(['is_on_sale'=>1,'cat_id'=>['in',implode(',', $cat_id_arr)]])->whereOr(['is_on_sale'=>1,'extend_cat_id'=>['in',implode(',', $cat_id_arr)]])->cache(true,1)->getField("goods_id",true);
        ...
        $filter_spec  = $goodsLogic->get_filter_spec_frontdesk($filter_goods_id,$filter_param,'goodsList',1); // 获取指定分类下的帅选规格
        $filter_attr  = $goodsLogic->get_filter_attr($filter_goods_id,$filter_param,'goodsList',1); // 获取指定分类下的帅选属性
//dump($filter_spec);die;
        ...
        if($count > 0)
        {
            ...
            if($filter_goods_id2)
            $goods_images = M('goods_images')->where("goods_id", "in", implode(',', $filter_goods_id2))->order("img_id asc")->cache(true)->select();
        }
        // print_r($filter_menu);
...
//        }
        ...
        if ($count > 0) {
            ...
            $filter_goods_id2 = get_arr_column($goods_list, 'goods_id');
            if ($filter_goods_id2)
                $goods_images = M('goods_images')->where("goods_id", "in", implode(',', $filter_goods_id2))->order('img_id asc')->select();
        }
        $point_rate = tpCache('shopping.point_rate');
        $this->assign('goods_list', $goods_list);
        $this->assign('goods_images', $goods_images);  // 相册图片
