...
class User extends Base{
    ...
    public function login(){
        ...
        $this -> assign('first_login',$_COOKIE['error_total']);
...
//            $cartLogic->login_cart_handle($this->session_id,$data['result']['user_id']);  //用户登录后 需要对购物车 一些操作
    ...
    */
    ...
    public function return_goods()
    {
        ...
        if(empty($order))
        {
            $this->error('非法操作,无该订单');
            exit;
        }
        $order_goods_where = ['order_id'=>$order_id,'goods_id'=>$goods_id,'spec_key'=>$spec_key];
        $return_goods = M('return_goods')->where($order_goods_where) -> order('id desc') -> find();
        $order_action_where = ['order_id' => $order_id];
        $status_desc = M('order_action') -> where($order_action_where) -> order('action_id desc') -> getField('status_desc');
        if(isset($return_goods) && $return_goods['status'] == 0)
        {
            $this->success('已经提交过退货申请且还没有审核结束!',U('Home/User/return_goods_info',array('id'=>$return_goods['id'])));
            exit;
        }
        //var_dump($order_action_where);die;
//        if($status_desc =='success'){
//            $this->success('已经提交过退货申请且审核通过了!',U('Home/User/return_goods_info',array('id'=>$return_goods['id'])));
//            exit;
//        }
        ...
        if(IS_POST)
        {
            ...
            $data['type'] = I('type'); // 服务类型  仅退款 退货 或者 换货
            $data['reason'] = I('reason'); // 问题描述
            $data['imgs'] = I('imgs'); // 用户拍照的相片
            $data['spec_key'] = I('spec_key'); // 商品规格
            $rate = round($order_goods['member_goods_price']*$order_goods['goods_num']/$order['goods_price'],2);
