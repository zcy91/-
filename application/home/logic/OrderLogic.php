...
class OrderLogic extends Model
{
    ...
    public function addOrder($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price,$user_note='',$pay_name='')
    {
        ...
        foreach($cartList as $key => $val)
        {
            ...
            $data2['market_price']       = $val['market_price']; // 专柜价
            ...
            //  M('goods')->where("goods_id = ".$val['goods_id'])->setInc('sales_sum',$val['goods_num']); // 商品添加销量
             $key = $val['spec_key_name'];
             $goods_id = $val['goods_id'];
             $store_count_arr =  M('spec_goods_price') -> where("key_name = '$key' and goods_id = '$goods_id'") -> find();
             if($store_count_arr)
                $arr['store_count'] = $store_count_arr['store_count'] - $val['goods_num'];
             else
                 $arr['store_count'] = $goods['store_count'] - $val['goods_num'];

             if($arr['store_count'] < 0){
                 $msg = $val['goods_name'] . '购买数量超过库存，请重新购买';
                 exit(json_encode(array('status'=>-10,'msg'=>"$msg",'result'=>null)));
             }
            // M('spec_goods_price') -> where("key_name = '$key' and goods_id = '$goods_id'") -> save($arr);
        }
...
//        M('goods_activity')->where(array('act_id'=>$act_id))->setInc('act_count',$pre_sell_price['goods_num']);
        ...
        $data2['market_price']       = $goods['market_price']; // 专柜价
    ...
    public function alterReturnGoodsInventory($order_id,$goods_id){
        ...
        if($spec_goods['prom_type']==2){    //是团购的
            ...
            if(!empty($spec_goods['spec_key'])){
                ...
                $key_name = M('spec_goods_price') -> where("goods_id = $goods_id and `key` = '{$spec_goods['spec_key']}'") ->
                getField('key_name');
                order_stock_log($order_id,$goods_id,1,$key_name,$spec_goods['goods_num']);
            }else{
                M('goods')->where("goods_id = $goods_id")->setInc('store_count', $spec_goods['goods_num']);//没有规格则增加商品库存
                order_stock_log($order_id,$goods_id,1,'',$spec_goods['goods_num']);
            }
        }
