...
class GoodsLogic extends Model
{
    ...
    /**
     * 用于前台显示时整合规则参数的方法，上面方法用于后台，不便于整合
     * @param     $goods_id_arr
     * @param     $filter_param
     * @param     $action
     * @param int $mode
     ...
     * @return array
     */
    public function get_filter_spec_frontdesk($goods_id_arr, $filter_param, $action, $mode = 0)
    {
        $goods_id_str = implode(',', $goods_id_arr);
        $goods_id_str = $goods_id_str ? $goods_id_str : '0';
        $spec_key = DB::query("select group_concat(`key` separator  '_') as `key` from __PREFIX__spec_goods_price where goods_id in($goods_id_str)");  //where("goods_id in($goods_id_str)")->select();
        ...
        foreach ($spec_key as $k => $v) {
            ...
            //将规则名称相同的项合并显示
            $key=array_search($spec[$spec_item[$v]['spec_id']],$tempName);
            if(!$key){
                $tempName[$spec_item[$v]['spec_id']]=$spec[$spec_item[$v]['spec_id']];
                $list_spec[$spec_item[$v]['spec_id']]['spec_id'] = $spec_item[$v]['spec_id'];
                $list_spec[$spec_item[$v]['spec_id']]['name'] = $spec[$spec_item[$v]['spec_id']];
            }
            ...
            $item=['key' => $spec_item[$v]['spec_id'], 'val' => $v, 'item' => $spec_item[$v]['item'], 'href' => urldecode(U("Goods/$action", $filter_param, ''))];
            if($key) {
                $list_spec[$key]['item'][] = $item;
            }else{
    ...
    function get_filter_price($goods_id_arr, $filter_param, $action, $c = 5)
    {
        ...
        $goods_id_str = implode(',', $goods_id_arr);
        $goods_id_str = $goods_id_str ? $goods_id_str : '0';
        $priceList = M('goods')->where(['is_on_sale'=>1,"goods_id"=>["in", $goods_id_str]])->getField('shop_price', true);  //where("goods_id in($goods_id_str)")->select();
        rsort($priceList);
    ...
    function getGoodsIdByBrandPrice($brand_id, $price)
    {
        ...
        if ($brand_id) // 品牌查询
        {
            ...
            $brand_select_goods = M('goods')->where(['is_on_sale'=>1])->whereIn('brand_id',$brand_id_arr,'or')->getField('goods_id', true);
        }
        if ($price)// 价格查询
        {
            $price = explode('-', $price);
    ...
    function getFilterSelected($sel ,$cat_id = array(1)){
        $where = " 1 = 1 ";
        $Goods = M('goods')->where(['is_on_sale'=>1,"cat_id"=>["in" ,implode(',', $cat_id)]]);
        //查看全部
        if($sel == 'selall'){
            $where .= '';
        }
