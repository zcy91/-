...
//                    }
    ...
    public function addEditGoods()
    {
        ...
        if ((I('is_ajax') == 1) && IS_POST) {
            ...
            $data = input('post.');
            if($goods_id){
                M("goods_asks")->where("goods_id",$goods_id)->delete();
                $arr = [];
                foreach ($data['ask'] as $k=>$v){
                    if($v){
                        $arr[$k]['ask'] = trim($v);
                        $arr[$k]['goods_id'] = $goods_id;
                        $arr[$k]['addtime'] = time();
                        $arr[$k]['admin_id'] = session('admin_id');
                        $arr[$k]['answer'] = trim($data['answer'][$k]);
                        $arr[$k]['sort'] = trim($data['sort'][$k]);
                    }
                }
                M('goods_asks')->insertAll($arr);
                unset($data['ask']);
                unset($data['answer']);
            }
...
//            }
        ...
        $asks = M('goods_asks')->where('goods_id=' . I('GET.id', 0))->order("sort asc")->select();
