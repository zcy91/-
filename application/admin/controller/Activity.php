...
class Activity extends Base
{
    ...
    {
        $Ad = M('activity_group_buy')->where(['del_status' => 0]);
        $count = $Ad->count();
        $Page = new Page($count, 10);
        $res = $Ad->where(['del_status'=>0])->order('id desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
        if ($res) {
            foreach ($res as $val) {
    ...
    public function group_buy()
    {
        ...
        if ($groupbuy_id) {
            $group_info = D('activity_group_buy')->where('id=' . $groupbuy_id)->find();
    ...
    {
        $data = I('post.');
        $data['intro'] = htmlspecialchars(stripslashes($data['intro']));
        $data['start_time'] = strtotime($data['start_time']);
        $data['end_time'] = strtotime($data['end_time']);
        if ($data['act'] == 'del') {
            $r = M('activity_group_buy')->where('id=' . $data['id'])->save(['del_status'=>1]);
            //M('goods')->where("prom_type=2 and prom_id=" . $data['id'])->save(array('prom_id' => 0, 'prom_type' => 0));
            if ($r) exit(json_encode(1));
        }
//        $groupBuyValidate = Loader::validate('ActivityGroupBuy');
...
//        }
        if ($data['act'] == 'add') {
            $r = M('activity_group_buy')->add($data);
            //M('goods')->where("goods_id=" . $data['goods_id'])->save(array('prom_id' => $r, 'prom_type' => 2));
        }
        if ($data['act'] == 'edit') {
            $r = M('activity_group_buy')->where('id=' . $data['id'])->save($data);
           // M('goods')->where("prom_type=2 and prom_id=" . $data['id'])->save(array('prom_id' => 0, 'prom_type' => 0));
...
//        $cat_id = I('cat_id');
        ...
        if($key_word)
        {
            $where = "$where and (name like '%$key_word%' or phone = '$key_word' or company like '%$key_word%' or user_id = '$key_word')" ;
        }

        $count = M('activity_group_user')->alias('c')->join('__ACTIVITY_GROUP_BUY__ gc', 'gc.id=c.aid')->where($where)->count();
        ...
        $goodsList = M('activity_group_user')->alias('c')->join('__ACTIVITY_GROUP_BUY__ gc', 'gc.id=c.aid')->where($where)->limit($Page->firstRow.','.$Page->listRows)->select();
...
//        $this->assign('catList',$catList);
    ...
    public function addEditGroupUser(){
        if(IS_POST){
            $data = I('post.');
            $r = M("activity_group_user")->where("aid = $data[aid] and user_id = $data[user_id]")->save($data);
            if($r){
                $this->success('编辑成功', U('Activity/groupList',['id'=>$data['aid']]));
            }else{
                $this->error('编辑失败', U('Activity/groupList',['id'=>$data['aid']]));
            }
        }else{
            $user_id = I('user_id');
            $aid = I('aid');
            $data = M('activity_group_user')->alias('u')->join('__ACTIVITY_GROUP_BUY__ gc', 'gc.id=u.aid')->where("aid = $aid and user_id = $user_id")->find();
