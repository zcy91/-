...
class Order extends Base {
    ...
    public function ajaxindex(){
        ...
        $admin_id ? $condition['admin_id'] = $admin_id : $condition['admin_id'] > 0;
    ...
    */
    ...
    public function delivery_info(){
        ...
        if(!$orderGoods)$this->error('此订单服务已完成退款或更换');//已经完成售后的不能再发货
        ...
        if(IS_POST)
        {
           ...
           $order['total_amount']   = $result['result']['total_amount']; // 订单总价
            ...
            if($order_id)
            {
                ...
                $this->success('添加服务商品成功',U("Admin/Order/detail",array('order_id'=>$order_insert_id)));
                exit();
            }
            else{
                $this->error('添加失败');
