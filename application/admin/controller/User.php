...
class User extends Base {
    ...
    public function consult(){
        ...
        if($timegap){
            ...
            $begin = $gap[0];
            $end = $gap[1];
            $map['end_time'] = array('between',array(strtotime($begin),strtotime($end)));
        }
        if($nickname){
            $map['nickname'] = array('like',"%$nickname%");
        }
        if($admin_id !=1){
            $map['admin_id'] = array('=',$admin_id);
        }else{
            $admin_id = I('admin_id','');
            $admin_id ? $map['admin_id'] = $admin_id : $map['admin_id'] > 0;
        }

        $data = M('admin')->getField('user_name,admin_id');
        $this->assign('data',$data);
        $count = M('consult')->where($map)->count();
        $page = new Page($count);
        $lists  = M('consult')->where($map)->order("end_time asc")->limit($page->firstRow.','.$page->listRows)->select();
   ...
   public function consultedit(){
        if(IS_POST) {
            ...
            if($data['cid']){
                M('consult')->where('id='.$data['cid'])->save($data);
                M('consult_return')->where("cid = ".$data['cid'])->delete();
                $arr = [];
                foreach ($data['return_content'] as $k=>$v){
                    if($v){
                        $arr[$k]['return_content'] = $v;
                        $arr[$k]['cid'] = $data['cid'];
                        $arr[$k]['return_admin_id'] = session('admin_id');
                        $arr[$k]['addtime'] = time();
                        $arr[$k]['end_time'] = strtotime($data['end_time']);
                    }
                }
                $r = M('consult_return')->insertAll($arr);
            }else{
                $data['return_admin_id'] = session('admin_id');
                $data['addtime'] = time();
                $user_id = M("users")->where("mobile",$data['phone'])->getField('user_id',true);
                if(empty($user_id)){
                    $data['user_id'] = 0;
                }else{
                ...
                $data['cid'] = M('consult')->insertGetId($data);
                foreach ($data['return_content'] as $k=>$v){
                    if($v){
                        $arr[$k]['return_content'] = $v;
                        $arr[$k]['cid'] = $data['cid'];
            ...
            if($id){
                $data=[];
                $data['readtime'] = time();
                D('consult')->where('id='.$id)->save($data);
                $info = D('consult')->where('id='.$id)->find();
                $this->assign('info',$info);
                $lists = M("consult_return")->where('cid='.$id)->order("addtime asc")->select();
                $this->assign('lists',$lists);
            }
            $this->assign('act',$act);
            return $this->fetch();
    ...
    public function account_edit(){
        ...
        if($order_info['return_id']){  //有退货id,是订单退款
            ...
            $order = M('order') -> field('user_money,integral') -> where('order_id',$return_info['order_id']) -> find();
    ...
    {
        $model = M("withdrawals");
        $model->where('id ='.$_GET['id'])->save(['status'=>-2]);
    ...
    public function withdrawals_update(){
        ...
        if($status == 1){
            ...
            foreach($withdrawals as $val){
                if($val['status'] !=1){
                    $this->ajaxReturn(array('status'=>0,'msg'=>'未提现通过'),'JSON');
                }
                ...
                if($user['user_money'] < $val['money'])
                {
                    ...
                    if($val['bank_name'] == '微信'){
                        $openid = M('users')->where('user_id',$val['user_id'])->getField('openid');
                        $wxpay = array(
                            'userid' => $val['user_id'],//用户ID做更新状态使用
                            'openid' => $openid,//收钱的人微信 OPENID
                            'pay_code'=>'weixin'.time().rand(1,99999),//提现申请ID
                            'money' => $val['money'],//金额
                            'desc' => '恭喜您提现申请成功!'
                        );
                        ...
                        if($res['partner_trade_no']){
                            ...
                            $r = M('withdrawals')->where(array('id'=>$val['id']))->save(array('status'=>$status,'pay_time'=>time(),'pay_code'=>$wxpay['pay_code']));
                        }else{
