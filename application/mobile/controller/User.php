    ...
    */
    public function _initialize()
    {
        ...
        $order_status_coment = array(
            ...
            'WAITSEND' => '待处理', //订单查询状态 待发货
            'WAITRECEIVE' => '处理中', //订单查询状态 待收货
...
//        $this->success($data['msg']);
    ...
    public function add_comment()
    {
        if (IS_POST) {
            ...
            $admin_id = M('order')->where("order_id",$add['order_id'])->getField('admin_id');
...
//        $goods = M('order_goods')->where("order_id",$order_goods['order_id'])->select();
    ...
    public function ajaxZan()
    {
        ...
        if(empty($comment_info['zan_userid'])){
...
//        }
    ...
    public function withdrawals()
    {
        ...
        if (IS_POST) {
            ...
            $data['bank_card'] = $data['account_bank'];
            $data['realname'] = $data['account_name'];
...
//            $this->redirect(U('Mobile/User/paypwd', array('step' => 3)));
            exit;
        }
        $this->assign('step', $step);
        return $this->fetch();
