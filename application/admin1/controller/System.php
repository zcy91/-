...
class System extends Base
{
     ...
     * 手机导航
     */
    public function navigationListMobile(){
        $model = M("Navigationmobile");
        $navigationList = $model->order("id desc")->select();
        $this->assign('navigationList',$navigationList);
        return $this->fetch('navigationListMobile');
    }
     ...
     * 添加修改编辑 手机前台导航
     */
    public function addEditNavMobile()
    {
        ...
        if (IS_POST) {
            ...
            $this->success("操作成功!!!", U('Admin/System/navigationListMobile'));
            exit;
        }
        // 点击过来编辑时
        $id = I('id',0);
        $navigation = DB::name('navigationmobile')->where('id',$id)->find();
        // 系统菜单
        $GoodsLogic = new GoodsLogic();
        $cat_list = $GoodsLogic->goods_cat_list();
        $select_option = array();
        ...
        $this->assign('navigation', $navigation);
        return $this->fetch('_navigationmobile');
    }
    /**
     * 删除前台 自定义 导航
     */
     ...
     * 删除手机前台 自定义 导航
     */
    public function delNavMobile()
    {
        // 删除导航
        $id = I('id');
        $image = M('navigationmobile') -> where("id = $id") ->getField('image');//删除旧图片
        $file = 'public/images/'.$image;
        unlink($file);
        M('navigationmobile')->where("id",I('id'))->delete();
        $this->success("操作成功!!!",U('Admin/System/navigationListMobile'));
    }
    /**
     * 删除商品相册图
    ...
    {
        $path = I('filename','');
        M('navigationmobile')->where("image = '$path'")->delete();
    }
