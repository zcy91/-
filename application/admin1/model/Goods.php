...
class Goods extends Model {
    ...
    {
         // 商品货号
         $goods_sn = "YX".str_pad($goods_id,7,"0",STR_PAD_LEFT);
         $this->where("goods_id = $goods_id and goods_sn = ''")->save(array("goods_sn"=>$goods_sn)); // 根据条件更新记录
            $goods_video = I('site');
            //$videonum = M('GoodsVideos')->where("goods_id = $goods_id")->count();
            if($goods_video){
                M('GoodsVideos')->where("goods_id = $goods_id")->delete();
                $data = ['goods_id'=>$goods_id,'video_url'=>$goods_video];
                M('GoodsVideos')->insert($data);
            }
         // 商品图片相册  图册
