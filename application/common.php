...
function admin_name($admin_id){
    ...
    $name = M('admin')->where("admin_id",$admin_id)->getField('user_name');
    return $name;
}
/**
...
function articles_thum_images($goods_id, $width, $height)
{
    if (empty($goods_id)) return '';

    //判断缩略图是否存在
    $path = "public/upload/article/articles/thumb/$goods_id/";
    $goods_thumb_name = "articles_thumb_{$goods_id}_{$width}_{$height}";
    ...
    $original_img = M('Article')->cache(true, 3600)->where("article_id", $goods_id)->getField('thumb');
    if (empty($original_img)) {
        return '/public/images/icon_goods_thumb_empty_300.png';
    }
...
function videos_thum_images($goods_id, $width, $height)
{
    if (empty($goods_id)) return '';

    //判断缩略图是否存在
    $path = "public/upload/videos/thumb/$goods_id/";
    $goods_thumb_name = "videos_thumb_{$goods_id}_{$width}_{$height}";
    ...
    $original_img = M('Video')->cache(true, 3600)->where("article_id", $goods_id)->getField('thumb');
    if (empty($original_img)) {
        return '/public/images/icon_goods_thumb_empty_300.png';
    }
...
/**
 * 根据 goods_id 判断是否是报单商品
 * @param type $good_id  商品id
 */
function is_baodan($goods_id){
    $cat_id = M('Goods') -> where('goods_id', $goods_id) -> getField('cat_id');//上一级的分类
    $parent_id_path = M('goods_category') -> where('id',$cat_id) -> getField('parent_id_path');
    $category_array=explode('_',$parent_id_path);
    $category_id = (int)$category_array[1];//得到一级分类id
    $res = M('goods_category') -> where('id',$category_id) ->getField('name');
    if($res =='报单商城')
        return true;
    else
        return false;
...
function minus_stock($order_id){
    ...
    foreach($orderGoodsArr as $key => $val)
    {
        ...
        $goods_spec = M('SpecGoodsPrice') -> where(['goods_id'=>$val['goods_id'],'key'=>$val['spec_key']]) -> getField('key_name');
        order_stock_log($order_id,$val['goods_id'],'',$goods_spec,$val['goods_num']);
    }

}
//库存日志更新,$type :null-- 出订单 1--退单 mtype:-1-- 出库；1--入库
function order_stock_log($order_id='',$goods_id = '',$type='',$spec_key='',$goods_num=''){
    ...
    if (!empty($spec_key)) {
        // 先到规格表里面扣除数量 再重新刷新一个 这件商品的总数量
        //$goods_spec = M('SpecGoodsPrice') -> where(['goods_id'=>$goods_id,'key'=>$spec_key]) -> getField('key_name');
        $data['goods_spec'] = $spec_key;
    }else{
        $data['goods_spec'] = '';
    }
...
//库存日志更新 后台增减 $key:规格
function admin_stock_log($goods_id='',$key='',$oldStore='',$key_name=''){
    $data = array();
    $data['order_sn'] ='N/A';
    ...
    if($count == 0) // 没有使用规格方式
    {
        ...
        $store = $store_count - $oldStore;

        $data['stock'] = $store;
    }

    if(!empty($key)){
        $store_count = M("SpecGoodsPrice")->where(['goods_id' => $goods_id, 'key' => $key])->getField('store_count');
        if(!$key_name)
        $key_name = M("SpecGoodsPrice") -> where(['goods_id' => $goods_id,'key' => $key]) -> getField('key_name');
        $store =$store_count - $oldStore;

        $data['stock'] = $store;
        $data['goods_spec'] = $key_name;
    }
    if($data['stock'] != 0)
    M('stock_log') -> add($data);
...
//库存日志 后台操作前存储之前库存
function admin_old_stock($goods_id='',$key=''){
    $count = M("SpecGoodsPrice")->where("goods_id", $goods_id)->count();
    if($count == 0){
        $store_count = M('goods') -> where('goods_id',$goods_id) -> getField('store_count');
    }else{
...
{
    return strpos(var_export($array, true), sprintf("'%s',", $value)) >= 0 ? true : false;
}
function foo(&$v) {
    $v = rawurlencode($v);
}
 ...
 * @return  bool
 */
function accountApiLog($user_id, $user_money = 0,$pay_points = 0, $desc = '',$distribut_money = 0,$order_id = 0 ,$order_sn = ''){
    /* 插入帐户变动记录 */
    $account_log = array(
        'user_id'       => $user_id,
        'user_money'    => $user_money,
...
function orderStatusDesc($order_id = 0, $order = array())
{
    ...
    else // 非货到付款
    {
        if($order['pay_status'] == 0 && $order['order_status'] == 0)
            return 'WAITPAY'; //'待支付',
        if($order['pay_status'] == 1 &&  in_array($order['order_status'],array(0)) && $order['shipping_status'] == 0)
            return 'WAITSEND'; //'待发货',
        if($order['order_status'] == 1 && $order['shipping_status'] == 0)
            return 'PORTIONING'; //'处理中',
...
function calculate_price($user_id = 0, $order_goods, $shipping_code = '', $shipping_price = 0, $province = 0, $city = 0, $district = 0, $pay_points = 0, $user_money = 0, $coupon_id = 0, $couponCode = '')
{
    ...
    $result = array(
        ...
        'point' => $result['order_integral']
    );
    return array('status' => 1, 'msg' => "计算价钱成功", 'result' => $result); // 返回结果状态
}
 function getRate($admin_id,$goods_id){
 ...
 function getOrderNum($admin_id,$goods_id){
//        $map = [];
