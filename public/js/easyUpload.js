...
; (function ($) {
  $.fn.easyUpload = function (opts) {
    var defaults = {
      allowFileTypes: '*.pdf;*.doc;*.docx;*.jpg;*.mp4',//允许上传文件类型，格式'*.pdf;*.doc;'
      ...
      var easyManager = {
        ...
        _renderFile: function (fileArr, target) {
          ...
          function render(file) {
            ...
            var $html = '';
            var num = Number(file.index) + 1;
            $html += '<li class="easy_upload_queue_item" id="'+file.index+'" data-index="' + file.index +'">';
            $html += '<input name="mulusort[]" value="'+ num+'"  style="width: 20px" type="text">';
