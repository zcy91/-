...
class weixin extends Model
{
    ...
    public function  __construct() {
        ...
        WxPayConfig::$mchid = isset($config_value['mchid']) ? $config_value['mchid'] : ''; // * SMCHID：服务商商户号（必须配置，开户邮件中可查看）
    ...
    {
            $subject = M('order_goods') -> where('order_id',$order['order_id']) -> getField('goods_name');
            $goods_id = M('order_goods') -> where('order_id',$order['order_id']) -> getField('goods_id');
            ...
            $input->SetProduct_id($goods_id); // 商品ID trade_type=NATIVE，此参数必传。此id为二维码中包含的商品ID，商户自行定义。
    ...
    function getJSAPI($order){
        ...
        //②、统一下单
        $input = new WxPayUnifiedOrder();
		$goods_name = M('order_goods')->where('order_id',$order['order_id'])->select();
